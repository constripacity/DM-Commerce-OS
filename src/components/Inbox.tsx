...
interface InboxProps {
  ...
  onExportCSV?: () => void;
  orders?: any[];
  onForceMarkPaid?: (orderId: string) => void;
}

export function Inbox({ threads, selectedThreadId, onSelectThread, paymentLinksSent, ordersPaid, onExportCSV, orders = [], onForceMarkPaid }: InboxProps) {
      ...
      <div className="p-4 border-b border-gray-200">
        ...
        <div className={`mt-4 p-3 rounded-lg bg-gray-50 border ${themeClasses.border}`}>
          ...
          {orders && orders.length > 0 && (
            <div className="mb-2">
              <label className="text-xs block mb-1">Latest unpaid order</label>
              <div className="flex gap-2">
                ...
                >
                  ...
                  {orders.filter((o: any) => o.status !== 'paid').slice(0,10).map((o: any) => (
                    <option key={o.id} value={o.id}>
                      {o.id} — {o.total ? `€${o.total}` : ''} — {o.status}
                    </option>
                  ))}
                </select>
                <button
                  onClick={() => selectedOrderId && onForceMarkPaid && onForceMarkPaid(selectedOrderId)}
